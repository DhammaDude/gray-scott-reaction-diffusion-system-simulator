...
- **Lattice diagnostics** use a separate lighter-blur local-peak detector to quantify faint organization between spots: mean neighbor count, sixfold order, bridge intensity, defect rate, and mean spacing. The two overlays are separate but share the same detected spot centers and neighbor graph: `show inferred neighbor links` draws center-to-center links, while `show inferred hexagonal cells` draws local perpendicular-bisector cells to test a honeycomb-like, negative-space interpretation. Neither treats the geometry as literal chemical bonds; tracked links briefly fade through split/merge detection gaps.
- **Field view** uses a camera over the larger simulation grid: moving from `0.5×` toward `1.0×` widens the visible field and shows more, smaller spots; moving toward `0.5×` crops closer for inspection.
