...
The simulator now estimates this organization directly. Its lattice diagnostics use a separate lighter-blur local-peak detector and report mean neighbors, a sixfold-order score, bridge intensity between neighboring spot centers, defect rate, and mean spacing. A measured-edge overlay can be toggled on or off, and the `field view` camera can widen from `0.5×` toward `1.0×` to show more of the larger simulated field with smaller spots. Measured edges are matched across detection ticks and briefly faded rather than removed immediately when a spot splits, merges, or momentarily falls below threshold.
